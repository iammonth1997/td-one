...
function polygonContainsPoint(lat, lon, boundary) {
  ...
  for (let index = 0, previous = points.length - 1; index < points.length; previous = index, index += 1) {
    ...
    const intersects = ((currentPoint.lat > lat) !== (previousPoint.lat > lat))
      && (lon < ((previousPoint.lng - currentPoint.lng) * (lat - currentPoint.lat))
...
function derivePolygonRadius(boundary) {
  ...
  return Math.round(
    Math.max(
      ...boundary.points.map((point) => haversineMeters(center.latitude, center.longitude, point.lat, point.lng))
    )
  );
}
...
export async function getEmployeeFromSessionEmpId(empId) {
  ...
  return resolveEmployeeByCode(prisma, empId);
}
...
export async function loadActiveWorkLocations() {
  ...
  try {
    ...
    const rows = (data || []).map((row) => ({
      ...
      boundary_json: normalizeBoundaryType(row.boundary_type) === "rectangle"
        ? normalizeRectangleBoundary(row.boundary_json)
        : normalizeBoundaryType(row.boundary_type) === "polygon"
          ? normalizePolygonBoundary(row.boundary_json)
          : null,
    ...
    return {
      rows: rows.map((row) => {
        ...
        if (row.boundary_type === "polygon" && row.boundary_json) {
          const center = derivePolygonCenter(row.boundary_json);
          return {
            ...row,
            latitude: center.latitude,
            longitude: center.longitude,
            radius_meters: derivePolygonRadius(row.boundary_json),
          };
        }
...
export async function verifyWorkLocation(lat, lon) {
  ...
  for (const location of rows) {
    ...
    const withinBoundary = location.boundary_type === "rectangle" && location.boundary_json
      ...
      : location.boundary_type === "polygon" && location.boundary_json
        ? polygonContainsPoint(Number(lat), Number(lon), location.boundary_json)
...
export async function logAttendanceScanAttempt(payload) {
  const prisma = getPrisma({ DATABASE_URL: process.env.DATABASE_URL });
  try {
    await prisma.attendanceLog.create({ data: payload });
  } catch (error) {
