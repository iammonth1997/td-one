  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/payroll/runs", {
        ...
        body: JSON.stringify({ run_type: "salary", period_month: period, pay_date: payDate || null }),
      });
    ...
    <AdminShell title="Payroll Salary" session={loaderData.session}>
      <section className="overflow-hidden rounded-xl border border-[#d8dee8] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-[#e6ebf2] px-4 py-3">
          <h2 className="text-sm font-semibold text-[#1b2738]">Salary Runs ({rows.length})</h2>
          <button onClick={() => { setError(""); setShowModal(true); }} className="rounded-lg bg-[#2563eb] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#1d4ed8]">▶ New Salary Run</button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] text-sm">
            ...
            <tbody>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-6 text-center text-[#8a97ac]">No salary runs — click &quot;New Salary Run&quot; to start</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            <h3 className="mb-4 text-base font-semibold text-[#1b2738]">New Salary Run</h3>
